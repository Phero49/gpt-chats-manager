...
const downloadChat = () => {
  ...
  pdfMake.createPdf(dd, null).download(`${store.title}.pdf`);
};
const toPdf = async () => {
  $q.loading.show({
    message:
      "preparing your pdf this can take long if yo have code in your chat",
  });
  ...
  await new Promise((resolve) => {
    if (codes.length > 0) {
      for (let index = 0; index < codes.length; index++) {
        ...
        if (element.hasAttribute("class") == false) {
          ...
          const clonedParent =
            clonedEl.parentElement.parentElement.parentElement;
    ...
    await new Promise((resolve) => {
      allChildren.forEach((element, index) => {
        element.setAttribute("style", "lineHeight: 2");
        if (index + 1 >= allChildren.length) {
          resolve();
        }
      });
    });
  ...
  setTimeout(() => {
    $q.loading.hide();
    $q.dialog({
      message: "your pdf is ready download",
      ok: { label: "download", icon: "download", flat: true },
      dark: true,
    }).onOk(() => {
      downloadChat();
    });
