(() => {
          ...
          if (err.message === errorMessage) {
            if (!Array.isArray(currentMessage.payload)) {
              ...
              if (objectSize > this._maxMessageSize) {
                ...
                let data2 = currentMessage.payload;
                for (let i = 0; i < chunksRequired; i++) {
                  let take = Math.min(data2.length, arrayItemCount);
                  this.wall.send([{
  ...
  var db = class {
    ...
    async write(data2) {
      const collection = {};
      collection[this.collection] = data2;
    ...
    async hasCollection() {
      const collection = new Promise((resolve) => {
        chrome.storage.local.get(this.collection, async (item) => {
          if (item[this.collection] == void 0) {
            resolve(false);
          } else {
            resolve(true);
          }
    ...
    async InsertIntoCollection(key, data2, max) {
      if (data2 != void 0) {
          ...
          if (item != void 0) {
            if (all == false) {
              ...
              if (collection != null) {
                ...
                if (values.length > 0) {
                  const slice2 = values.slice(start, limit);
                  resolve(slice2);
                } else {
                  resolve([]);
                }
    ...
    async insertAoutoIndex(data2, max) {
      chrome.storage.local.get(this.collection, (item) => {
        const col = item[this.collection];
        if (max == void 0) {
          col[Object.keys(col).length] = data2;
          this.write(col);
        }
      });
    }
  ...
  var myCollections = { "my_chats": "my_chats", "chats_keys": "chats_keys", "recent": "recent", "folders": "folders", "folder_items": "folder_items" };
  ...
  var background_default = (0, import_wrappers.bexBackground)((bridge) => {
    async function processChat(message, sender) {
      try {
        ...
        if (sender.origin === "https://chat.openai.com" && regex.test(sender.url) && sender.url.includes("?model=text-davinci-002-render-sha") === false) {
          ...
          var groups = [];
          var chatData = [];
          for (let index2 = 0; index2 < chat_data.length; index2++) {
            const chatgptResponse = chat_data[index2];
            if ((index2 + 1) % 2 !== 0) {
              var $2 = load(chatgptResponse);
              var text3 = $2.root().text();
              const quation = `<div class="question itemCount-${index2 + 1}" style="font-weight:bold;font-size:18px;"> <p> ${text3} <p> </div>`;
              groups.push(quation);
              askedQuestions.push(text3);
            } else {
              groups.push(`<div class="response  itemCount-${index2 + 1}"> ${chatgptResponse} </div>`);
            }
            if (groups.length >= 2) {
              const group = `<div id="group-${index2 - 1}"> ${groups.join("")} </div>`;
              chatData.push(group);
              groups = [];
            }
          }
    ...
    bridge.on("getCollections", async ({ data: data2, respond }) => {
      const { all } = data2;
      const database = new db(myCollections.folders);
    ...
    bridge.on("getCollectionItems", async ({ data: data2, respond }) => {
      const { key } = data2;
      const database = new db(myCollections.folders);
      const results = await database.getOne(key);
      respond(results);
    });
    bridge.on("addTocollection", async ({ data: data2, respond }) => {
      const { collectionName } = data2;
      const { itemData } = data2;
      const { key } = itemData;
      const database = new db(myCollections.folders);
    ...
    bridge.on("deleteCollection", async ({ data: data2, respond }) => {
      const { key } = data2;
      const database = new db(myCollections.folders);
    ...
    bridge.on("getSingleChat", async ({ data: data2, respond }) => {
    ...
    bridge.on("getChats", async ({ respond }) => {
      function groupData(dataArray) {
        ...
        for (const chat of dataArray) {
          ...
          if (groupedChats[onlydate] == void 0) {
    ...
    bridge.on("removeFromCollection", async ({ data: data2, respond }) => {
      const { collectionName, url } = data2;
      const database = new db(myCollections.folders);
    ...
    bridge.on("addToRecent", async ({ data: data2, respond }) => {
      const { key, label } = data2;
    ...
    bridge.on("removeChat", async ({ data: data2, respond }) => {
      const { url } = data2;
    ...
    bridge.on("saveChats", async ({ data: data2, respond }) => {
      const { key, content } = data2;
      const database = new db(myCollections.my_chats);
      if (key != void 0) {
  ...
  chrome.runtime.onConnect.addListener((port) => {
    ...
    const bridge = new Bridge({
      ...
      send(data2) {
        for (let connectionId in connections) {
          const connection = connections[connectionId];
          connection.app && connection.app.connected && connection.app.port.postMessage(data2);
          connection.contentScript && connection.contentScript.connected && connection.contentScript.port.postMessage(data2);
        }
      }
    });
